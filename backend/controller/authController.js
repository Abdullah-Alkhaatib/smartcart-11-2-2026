...
const login = async (req, res) => {
    ...
    try {
        const user = await User.findOne({email}).select('+password') // عشان نجيب الباسورد مع بيانات المستخدم عشان نتحقق منهم
        if(!user) {
        ...
        const accessToken = user.generateAccessToken(); // عشان نولد توكن جديد لما المستخدم يسجل دخول
        const refreshToken = user.generateRefreshToken(); // عشان نولد توكن جديد لما المستخدم يسجل دخول

        user.refreshToken = refreshToken; // عشان نخزن الريفرش توكن في الداتا بيز
        await user.save();

        res.json({ _id: user._id, username: user.username, email: user.email, role: user.role, accessToken, refreshToken }); // نرجع بيانات المستخدم مع التوكن
